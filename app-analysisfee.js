...
(() => {
  ...
  setStatus=function(r,newStatus,reason=''){
    ...
    if(ok && old==='Binnengebracht' && newStatus==='Diagnose' && !r.analysisFeeEarned){r.analysisFeeEarned=true;log(r,'Analysekost geactiveerd','Niet verschuldigd',euro(feeOf(r)),'systeem','billing');}
    if(ok && newStatus==='Klaar zonder reparatie' && !r.analysisFeeEarned){r.analysisFeeEarned=true;log(r,'Analysekost geactiveerd','Niet verschuldigd',euro(feeOf(r)),'systeem','billing');}
    return ok;
  };
  ...
  calc=function(r){const c=originalCalc(r),fee=feeOf(r);return {...c,analysisFee:fee,total:c.parts+c.labor+c.other+fee};};
  ...
  newRepair=function(){originalNewRepair();const oc=$('oc');if(oc){const h=oc.querySelector('h3');if(h)h.insertAdjacentHTML('beforebegin',feeNotice());const createBtn=oc.querySelector('[data-action="repair-create"]');if(createBtn)createBtn.insertAdjacentHTML('beforebegin',`<label style="margin:12px 0"><input id="analysisFeeAcceptedDesk" class="check" type="checkbox"> Klant is geïnformeerd en gaat akkoord met de analysekost van <b>${euro(configuredFee)}</b> zodra de diagnose wordt uitgevoerd.</label>`);}};
  ...
  tabContent=function(ticket,r,c){
    let html=originalTabContent(ticket,r,c),fee=feeOf(r);
    ...
    if(activeRepairTab==='payment'){
      const q=currentQuote(r),basis=q&&q.status==='approved'?`Goedgekeurde offerte incl. analyse: ${euro(activeBillableTotal(r))}`:`Alleen analysekost verschuldigd: ${euro(analysisEarned(r)?fee:0)}`;
  ...
  overviewActions=function(ticket,r){let html=originalOverviewActions(ticket,r)||'',q=currentQuote(r);if((r.status==='Wachten op klant'&&(r.outcome==='Niet herstelbaar'||q?.status==='rejected'))||r.status==='Diagnose'&&r.outcome==='Niet herstelbaar') html+=` <button class="btn r" data-action="close-no-repair" data-ticket="${esc(ticket)}">Afronden zonder reparatie</button>`;return html;};

  document.addEventListener('click',e=>{
    const a=e.target.closest('[data-action]');if(!a)return;
    if(a.dataset.action==='repair-create'){const cb=$('analysisFeeAcceptedDesk');if(cb&&!cb.checked){e.preventDefault();e.stopImmediatePropagation();toast('Bevestig eerst dat de klant akkoord is met de analysekost.');return;}}
    if(a.dataset.action==='repair-save-quote'){const r=repairs[a.dataset.ticket],amount=+$('rqAmount')?.value;if(r&&amount<feeOf(r)){e.preventDefault();e.stopImmediatePropagation();toast('Offerte kan niet lager zijn dan de analysekost van '+euro(feeOf(r))+'.');}}
    if(a.dataset.action==='close-no-repair'){e.preventDefault();e.stopImmediatePropagation();const r=repairs[a.dataset.ticket];if(!r)return;if(!r.analysisFeeEarned){r.analysisFeeEarned=true;log(r,'Analysekost geactiveerd','Niet verschuldigd',euro(feeOf(r)),'systeem','billing');}if(setStatus(r,'Klaar zonder reparatie','Niet herstelbaar of klant ziet af van reparatie')){save();activeRepairTab='payment';detail(a.dataset.ticket,'payment');toast('Dossier klaar voor retour · alleen analysekost blijft open.');}}
    if(a.dataset.action==='pickup-no-repair'){e.preventDefault();e.stopImmediatePropagation();const r=repairs[a.dataset.ticket];if(!r)return;if(due(r)>0){toast('Analysekost moet eerst volledig betaald zijn.');return;}if(r.status!=='Klaar zonder reparatie'){toast('Dossier staat niet klaar voor retour.');return;}if(setStatus(r,'Afgehaald','Retour zonder reparatie')){if(r.unlock){r.unlock='';log(r,'Toestelcode automatisch gewist','[ingesteld]','[gewist]','systeem','security');}r.warranty=false;log(r,'Toestel teruggegeven zonder reparatie','','Analysekost voldaan','Amine','balie');save();detail(a.dataset.ticket,'overview');toast('Toestel teruggegeven · dossier afgerond.');}}
    if(a.dataset.action==='analysis-fee-settings'){e.preventDefault();e.stopImmediatePropagation();overlay('Analysekost instellen',`<div class="oc"><label>Standaard analysekost (€)</label><input id="analysisFeeSetting" type="number" min="0" step="0.01" value="${configuredFee.toFixed(2)}"><p class="small muted">Dit nieuwe bedrag geldt voor nieuwe dossiers. Bestaande dossiers behouden het bedrag dat bij hun intake werd vastgelegd.</p><button class="btn g" data-action="analysis-fee-save">Opslaan</button></div>`);}
    if(a.dataset.action==='analysis-fee-save'){e.preventDefault();e.stopImmediatePropagation();const v=+$('analysisFeeSetting')?.value;if(!(v>=0)){toast('Vul een geldig bedrag in.');return;}configuredFee=v;localStorage.setItem(FEE_KEY,String(v));closeOverlay();toast('Standaard analysekost ingesteld op '+euro(v)+'.');}
  },true);

  const originalRepairShell=repairShell;
  repairShell=function(){originalRepairShell();const host=document.querySelector('#repairs .toolbar');if(host&&!host.querySelector('[data-action="analysis-fee-settings"]')) host.insertAdjacentHTML('afterbegin',`<button class="btn s" data-action="analysis-fee-settings">Analysekost ${euro(configuredFee)}</button>`);};

  save();repairShell();if(document.querySelector('#repairs.page.active')) renderRepairList();console.log('RepairFlow analysekost V3.1 loaded');
})();